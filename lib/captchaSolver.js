...
import { storePjnSession } from './pjnSessionCache.js';

// URL oficial del pack tar de @sparticuz/chromium v143 para x64 (Linux/Vercel)
// chromium.executablePath() descarga este tar, lo extrae en /tmp y retorna el path
const CHROMIUM_REMOTE_URL =
    'https://github.com/Sparticuz/chromium/releases/download/v143.0.0/chromium-v143.0.0-pack.x64.tar';

/**
 * Lanza un browser compatible con el entorno actual.
 * - Vercel/Lambda: usa @sparticuz/chromium (descarga binario a /tmp)
 * - Local dev:    usa Chrome/Chromium instalado en el sistema
 */
async function launchBrowser() {
    const isVercel = !!process.env.VERCEL || !!process.env.AWS_LAMBDA_FUNCTION_NAME;

    if (isVercel) {
        // En Vercel necesitamos executablePath con URL al tar del binario
        // chromium.executablePath() extrae el brotli a /tmp y devuelve el path
...
async function runSCWSearch({ tab, jurisdiccion, jurisdictionName, numero, anio, nombre, parteTipo, maxPages = null }) {
    ...
    try {
        ...
        await page.waitForFunction(() => {
            // Esperar filas con formato de expediente (nro/año) en alguna tabla
            const trs = [...document.querySelectorAll('table tbody tr')];
            const hasExpedienteRow = trs.some(tr => {
                const cells = tr.querySelectorAll('td');
                return cells.length >= 4 && /\d+\/\d{4}/.test(cells[0]?.textContent || '');
            });
            if (hasExpedienteRow) return true;
            // O un mensaje de "sin resultados" / error visible
            const msgs = [...document.querySelectorAll(
                '[class*="rich-messages"] [class*="message"], [class*="rich-messages"] [class*="Message"], ' +
                '[class*="dataTable"] [class*="empty"], [id*="messages"], [id*="Messages"]'
            )];
            return msgs.some(el => (el.textContent?.trim().length || 0) > 5);
        }, { timeout: 30000 }).catch(() => {
...
export async function getExpedienteDetalle({ jurisdiccion, numero, anio }) {
    ...
    try {
        ...
        // Override window.open ANTES del click: el PJN SCW usa window.open() en JSF,
        // lo que abre una nueva pestaña que Puppeteer no sigue. Al redirigir en el tab actual
        // evitamos que la página quede en about:blank.
        console.log('[DeepScraper] Haciendo clic en el link del expediente para navegar a detalle...');
        await page.evaluate(() => {
            window.open = (url) => { if (url) window.location.href = url; return window; };
        });
        await Promise.all([
        ...
        // Extraer cookies de la sesión activa para poder proxear documentos
        try {
            const cookies = await page.cookies();
            const cookieHeader = cookies.map(c => `${c.name}=${c.value}`).join('; ');
            if (cookieHeader) {
                detalle.sessionToken = storePjnSession(cookieHeader);
                console.log(`[DeepScraper] Sesión guardada en cache (token: ${detalle.sessionToken.slice(0, 8)}...)`);
            }
        } catch (cookieErr) {
            console.warn('[DeepScraper] No se pudieron extraer cookies de sesión:', cookieErr.message);
        }
