...
export const PLAN_LIMITS = {
  ...
  /**
   * Miembro de estudio jurídico invitado por el owner.
   * Las quotas individuales son ilimitadas porque el consumo real
   * se descuenta del pool compartido de la organización (org_research_credits, etc.).
   */
  enterprise_member: {
    ai_messages: -1,
    inquiries: -1,
    ...
    features: {
      ...
      case_library: true,
      team_collaboration: true,
      api_access: false,       // Solo el owner/admin del estudio
      priority_support: true,
      custom_branding: false,  // Solo el estudio como entidad
      analytics: true,
      sla_guarantee: true,
      dedicated_support: true
    }
...
export function canPerformAction(profile, action) {
  ...
  const extra = profile[`${action}_extra`] || 0;
  // Cuota mensual agotada pero tiene créditos extra (pool depletable)
...
export function getUsageInfo(profile, action) {
  ...
  const used = profile[`${action}_used`] || 0;
  const extra = profile[`${action}_extra`] || 0;
  // Restante mensual + extra como pool separado
  const monthlyRemaining = limit === -1 ? Infinity : Math.max(0, limit - used);
  const remaining = limit === -1 ? Infinity : monthlyRemaining + extra;
...
export function getPlanName(planTier) {
  const names = {
    ...
    enterprise_member: 'Miembro Enterprise',
    enterprise_s: 'Enterprise S',
    enterprise_m: 'Enterprise M',
    enterprise_l: 'Enterprise L',
    enterprise_xl: 'Enterprise XL',
    pending_enterprise: 'Enterprise (Pendiente)'
  };
...
export function isValidPlanTier(planTier) {
  return ['trial', 'free', 'professional', 'enterprise', 'enterprise_member',
    'enterprise_s', 'enterprise_m', 'enterprise_l', 'enterprise_xl',
    'pending_enterprise'].includes(planTier);
}
