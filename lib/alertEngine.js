...
import { searchCABAByCaratula } from './cabaAlertSolver.js';
...
async function safeInsertMatches(supabase, { alertId, userId, portal, newMatches = [] }) {
    if (!newMatches.length) return;
    const rows = newMatches.map((m) => ({
        alert_id: alertId,
        user_id: userId,
        portal,
        expediente: cleanText(m?.expediente || m?.nroExpediente || ''),
        caratula: cleanText(
            m?.caratula || m?.autos ||
            [m?.denunciantes, m?.denunciados].filter(Boolean).join(' c/ ') || ''
        ).slice(0, 200),
        link: cleanText(m?.link || m?.url || ''),
        source: cleanText(m?.source || portal),
    }));
    const { error } = await supabase.from('case_alerts_matches').insert(rows);
    if (error) {
        console.error(`[AlertEngine] No se pudo guardar matches alerta ${alertId}:`, error.message);
    }
}

async function sendWhatsAppAlert({ profile, alert, newCount, sampleMatches = [] }) {
    const phone = profile?.whatsapp_phone;
    if (!phone || profile?.whatsapp_sub_status !== 'active') return false;

    const evoUrl = process.env.EVO_URL;
    const evoKey = process.env.EVO_APIKEY;
    ...
    const matchLines = (sampleMatches || []).slice(0, 5).map((item) => {
        const exp = cleanText(item?.expediente || item?.nroExpediente || '');
        const car = cleanText(
            item?.caratula ||
            [item?.denunciantes, item?.denunciados].filter(Boolean).join(' c/ ') ||
            ''
        );
        const dep = cleanText(item?.dependencia_asignada || item?.organismo || '');
        const fecha = cleanText(item?.assigned_date || item?.fecha || '');
        const parts = [];
        if (exp) parts.push(`*${exp}*`);
        if (car) parts.push(car.length > 60 ? car.slice(0, 57) + '…' : car);
        if (dep) parts.push(`_${dep}_`);
    ...
    const text =
        ...
        `Búsqueda: *${target}*\n` +
        `Portal: ${portalLabel}\n` +
        `Novedades nuevas: *${newCount}*\n\n` +
        (matchLines ? `${matchLines}\n\n` : '') +
        `Ver todas: ${dashboardUrl}`;
    ...
    try {
        ...
        if (!res.ok) {
            console.error(`[AlertEngine] WhatsApp send failed (${res.status}) for ${phone}`);
            return false;
        }
        return true;
    } catch (err) {
        console.error('[AlertEngine] WhatsApp send error:', err?.message || err);
        return false;
    }
}
...
export async function processDailyAlerts(options = {}) {
    ...
    if (dueUserIds.length > 0) {
        const { data: profiles, error: profileError } = await supabase
            .from('profiles')
            .select('id, email, full_name, whatsapp_phone, whatsapp_sub_status, push_subscription')
            .in('id', dueUserIds);
    ...
    for (const alert of dueAlerts) {
            ...
            if (!searchQuery) {
                ...
                runMeta = {
                    ...buildAlertRunMeta({
                        ...
                        samples: dedupeResult.new_matches,
                    }),
                ...
                const cabaResult = await searchCABAByCaratula({
                    nombre: searchQuery,
                    maxPages: 3,
                });

                if (cabaResult.error) throw new Error(cabaResult.error);

                const matches = (cabaResult.results || []).map((r) => ({
                    ...r,
                    id: cleanText(r.id || r.expediente || r.caratula),
                    dependencia_asignada: r.dependencia_asignada || 'TSJ CABA',
        ...
        if (runStatus === 'success' && emailPayload?.newCount > 0) {
            ...
            const pushPayload = {
                title: 'Nueva novedad en tu alerta',
                body: `${emailPayload.newCount} nuevas coincidencias en "${searchQuery}"`,
                url: '/dashboard/research',
            };

            const notifResults = await Promise.allSettled([
                safeInsertMatches(supabase, {
                    alertId: alert.id,
                    userId: alert.user_id,
                    portal,
                    newMatches: emailPayload.sampleMatches || [],
                }),
                notifyAlertByEmail({
                    resendClient,
                    profile,
                ...
                profile?.push_subscription
                    ? sendPushNotification(profile.push_subscription, pushPayload)
                    : Promise.resolve(false),
            ]);

            if (notifResults[1].status === 'fulfilled' && notifResults[1].value) summary.emails_sent += 1;
            if (notifResults[1].status === 'rejected') { summary.email_errors += 1; console.error(`[AlertEngine] Email error ${alert.id}:`, notifResults[1].reason?.message); }
            if (notifResults[2].status === 'rejected') console.error(`[AlertEngine] WA error ${alert.id}:`, notifResults[2].reason?.message);
            if (notifResults[3].status === 'rejected') console.error(`[AlertEngine] Push error ${alert.id}:`, notifResults[3].reason?.message);
        }
