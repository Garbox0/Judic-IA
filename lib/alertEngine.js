...
export async function processDailyAlerts(options = {}) {
    ...
    for (const alert of dueAlerts) {
        ...
        if (runStatus === 'success' && emailPayload?.newCount > 0) {
            await safeInsertMatches(supabase, {
                alertId: alert.id,
            ...
            // WhatsApp push if user has active WA subscription
            try {
                ...
                console.error(`[AlertEngine] Error WhatsApp alerta ${alert.id}:`, waErr?.message || waErr);
            }
        }
