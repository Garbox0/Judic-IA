/**
 * POST /api/mp/org-research/create
 *
 * Inicia una preferencia de pago en MercadoPago para comprar créditos
 * de Investigación (Estrategia IA) para el pool del estudio.
 * Solo disponible para el owner (Titular) del estudio verificado.
 *
 * Body: { pack_id: 'pack_5' | 'pack_15' | 'pack_30' }
 */
import { NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import { verifyAuth } from '@/lib/api-auth';

const ORG_RESEARCH_PACKS = {
    pack_5:  { credits: 5,  amount: 25000, label: 'Pack 5 reportes IA (estudio)' },
    pack_15: { credits: 15, amount: 60000, label: 'Pack 15 reportes IA (estudio)' },
    pack_30: { credits: 30, amount: 99000, label: 'Pack 30 reportes IA (estudio)' },
};
...
export async function POST(request) {
    ...
    const pack = ORG_RESEARCH_PACKS[pack_id];
    if (!pack) {
    ...
    if (dbError) {
        console.error('[mp/org-research/create] DB error:', dbError);
    ...
    const preference = {
        ...
        // Format: "org_research_credits:<purchase_id>:<org_id>:<user_id>"
        external_reference: `org_research_credits:${purchase.id}:${orgId}:${userId}`,
        back_urls: {
            success: `${appUrl}/dashboard/estudio/creditos?research_credits=ok`,
            failure: `${appUrl}/dashboard/estudio/creditos?research_credits=error`,
            pending: `${appUrl}/dashboard/estudio/creditos?research_credits=pending`,
        },
    ...
    if (!mpRes.ok) {
        console.error('[mp/org-research/create] MP error:', mpData);
        await supabase.from('org_research_credit_purchases').delete().eq('id', purchase.id);
