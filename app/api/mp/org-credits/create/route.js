/**
 * POST /api/mp/org-credits/create
 *
 * Inicia una preferencia de pago en MercadoPago para comprar créditos
 * de Antecedentes Judiciales para el pool del estudio.
...
const ORG_ANTECEDENTES_PACKS = {
    pack_5:  { credits: 5,  amount: 25000, label: 'Pack 5 importaciones (estudio)' },
    pack_15: { credits: 15, amount: 60000, label: 'Pack 15 importaciones (estudio)' },
    pack_30: { credits: 30, amount: 99000, label: 'Pack 30 importaciones (estudio)' },
};
...
export async function POST(request) {
    ...
    const pack = ORG_ANTECEDENTES_PACKS[pack_id];
    if (!pack) {
    ...
    if (dbError) {
        console.error('[mp/org-credits/create] DB error:', dbError);
    ...
    const preference = {
        ...
        // Format: "org_antecedentes_credits:<purchase_id>:<org_id>:<user_id>"
        external_reference: `org_antecedentes_credits:${purchase.id}:${orgId}:${userId}`,
        back_urls: {
            success: `${appUrl}/dashboard/estudio/creditos?credits=ok`,
            failure: `${appUrl}/dashboard/estudio/creditos?credits=error`,
            pending: `${appUrl}/dashboard/estudio/creditos?credits=pending`,
        },
    ...
    if (!mpRes.ok) {
        console.error('[mp/org-credits/create] MP error:', mpData);
        await supabase.from('org_antecedentes_credit_purchases').delete().eq('id', purchase.id);
