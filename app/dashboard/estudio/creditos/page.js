...
import { Coins, ShoppingCart, CheckCircle2, XCircle, Clock, User, AlertCircle } from 'lucide-react';
...
export default function EstudioCreditosPage() {
    ...
    const load = useCallback(async () => {
        ...
        const { data: org } = await supabase
            .from('organizations')
            .select('antecedentes_credits_pool')
            .eq('id', prof.org_id)
            .single();
    ...
    const handleBuy = async (packId) => {
        ...
        try {
            ...
            console.error('[EstudioCreditos]', err.message);
        ...
        <div className="estudio-page">
            <div className="estudio-page-header">
                ...
                <p className="estudio-page-subtitle">
                    Pool compartido del estudio — cada importación de expediente consume 1 crédito.
                </p>
            </div>
            ...
            {creditsStatus === 'ok' && (
                <div className="estudio-alert estudio-alert--success" style={{ marginBottom: '1.5rem' }}>
                    ...
                    Pago aprobado. Los créditos ya están disponibles en el pool del estudio.
                </div>
            )}
            {creditsStatus === 'error' && (
            ...
            {creditsStatus === 'pending' && (
                <div className="estudio-alert estudio-alert--warning" style={{ marginBottom: '1.5rem' }}>
                    <Clock size={16} />
                    El pago está pendiente de confirmación. Los créditos se acreditarán automáticamente.
                </div>
            )}
            ...
            <div className="estudio-stats-grid" style={{ marginBottom: '2rem' }}>
                <div className="estudio-stat-card" style={{ gridColumn: '1 / -1', maxWidth: 300 }}>
                    <div className="estudio-stat-icon" style={{ background: pool > 0 ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)' }}>
                        <Coins size={22} color={pool > 0 ? '#22c55e' : '#ef4444'} />
                    </div>
                    <div>
                            ...
                            <tbody>
                                {purchases.map(p => (
                                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                        ...
                                        <td style={{ padding: '0.6rem 0.75rem', textAlign: 'right', color: 'var(--text-secondary)' }}>
            ...
            {activeTab === 'uso' && (
                usage.length === 0 ? (
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                        Todavía no se usaron créditos del estudio para importar expedientes.
                    </p>
                ) : (
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                                    ...
                                    <th style={{ textAlign: 'left', padding: '0.5rem 0.75rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Expediente</th>
                                </tr>
                            </thead>
                            <tbody>
                                {usage.map(u => (
                                    <tr key={u.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                        ...
                                        <td style={{ padding: '0.6rem 0.75rem', color: 'var(--text-secondary)', fontFamily: 'monospace' }}>
                                            {u.expediente || '—'}
                                        </td>
                                    </tr>
                                ))}
