...
export default function EstudioCreditosPage() {
    ...
    const alertCreditsStatus = searchParams?.get('alert_credits');
    ...
    // ── Alertas ──
    const [alertPool, setAlertPool] = useState(null);
    const [alertPurchases, setAlertPurchases] = useState([]);
    const [alertUsage, setAlertUsage] = useState([]);
    const [buyingAlert, setBuyingAlert] = useState(false);
    const [activeAlertTab, setActiveAlertTab] = useState('compras');
    ...
    const load = useCallback(async () => {
        ...
        const { data: org } = await supabase
            .from('organizations')
            .select('antecedentes_credits_pool, research_credits_pool, alert_credits_pool')
            .eq('id', prof.org_id)
            .single();
        ...
        setAlertPool(org?.alert_credits_pool ?? 0);

        // Fetch all history tables in parallel
        const [
            { data: purchaseData },
            { data: usageData },
            { data: researchPurchaseData },
            { data: researchUsageData },
            { data: alertPurchaseData },
            { data: alertUsageData },
        ] = await Promise.all([
            supabase.from('org_antecedentes_credit_purchases')
            ...
            supabase.from('org_antecedentes_credit_usage')
                .select('id, expediente, used_at, used_by, profiles!used_by(full_name)')
                .eq('org_id', prof.org_id).order('used_at', { ascending: false }).limit(50),
            supabase.from('org_research_credit_purchases')
                .select('id, pack_id, credits, amount_ars, status, created_at, purchased_by, profiles!purchased_by(full_name)')
                .eq('org_id', prof.org_id).order('created_at', { ascending: false }).limit(30),
            supabase.from('org_research_credit_usage')
                .select('id, query, used_at, used_by, profiles!used_by(full_name)')
                .eq('org_id', prof.org_id).order('used_at', { ascending: false }).limit(50),
            supabase.from('org_alert_credit_purchases')
                .select('id, pack_id, credits, amount_ars, status, created_at, purchased_by, profiles!purchased_by(full_name)')
                .eq('org_id', prof.org_id).order('created_at', { ascending: false }).limit(30),
            supabase.from('org_alert_credit_usage')
                .select('id, alert_id, used_at, used_by, profiles!used_by(full_name)')
                .eq('org_id', prof.org_id).order('used_at', { ascending: false }).limit(50),
        ]);
        ...
        setAlertPurchases(alertPurchaseData || []);
        setAlertUsage(alertUsageData || []);
    ...
    // Reload data when returning from MercadoPago with approved payment
    useEffect(() => {
        if (creditsStatus === 'ok' || researchCreditsStatus === 'ok' || alertCreditsStatus === 'ok') {
            load();
        }
    }, [creditsStatus, researchCreditsStatus, alertCreditsStatus, load]);
    ...
    const handleBuyAlert = async (packId) => {
        setBuyingAlert(true);
        try {
            const { data: { session } } = await supabase.auth.getSession();
            const res = await fetch('/api/mp/org-alerts/create', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token}` },
                body: JSON.stringify({ pack_id: packId }),
            });
            ...
            console.error('[EstudioCreditos/alerts]', err.message);
            alert(`Error: ${err.message}`);
        } finally {
    ...
    return (
        <div className="estudio-page">
            ...
            {alertCreditsStatus === 'ok' && (
                <div className="estudio-alert estudio-alert--success" style={{ marginBottom: '1.5rem' }}>
                    <CheckCircle2 size={16} /> Pago aprobado. Los créditos de alertas ya están disponibles.
                </div>
            )}
            {alertCreditsStatus === 'error' && (
                <div className="estudio-alert estudio-alert--error" style={{ marginBottom: '1.5rem' }}>
                    <XCircle size={16} /> El pago no pudo procesarse. Podés intentarlo nuevamente.
                </div>
            )}
            {alertCreditsStatus === 'pending' && (
                <div className="estudio-alert estudio-alert--warning" style={{ marginBottom: '1.5rem' }}>
                    <Clock size={16} /> El pago está pendiente de confirmación. Los créditos se acreditarán automáticamente.
                </div>
            )}
