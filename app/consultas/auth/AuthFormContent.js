    ...
    useEffect(() => {
        ...
        const checkLinkValidity = async () => {
            if (cid) {
                ...
                // If the inquiry was deleted (but it's not a brand new link generated by the dashboard)
                // Actually, how to distinguish? 
                // A freshly generated link will NOT be in the DB yet. 
                // BUT if the client successfully registered once, it WAS linked.
                // If it's linked to an auth user, and that user is gone, it means it's a "zombie" link.

                // For now: if cid is present and not found, and we are in "register" mode, 
                // we only block if the client *already* had an account or if it's explicitly marked.
                // Simplify: A deleted inquiry leaves no trace. 
                // But we can check if the lawyerId is valid at least.
                if (lawyerId) {
