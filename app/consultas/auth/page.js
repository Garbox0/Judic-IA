    ...
    useEffect(() => {
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
            if (event === 'SIGNED_IN' && session) {
                ...
                if (lawyerId) {
                    // Try to create inquiry (idempotent due to logic in API or check)
                    // If returning from email, we might have lost "cid" state if not in URL?
                    // WE MUST GET CID FROM URL PARAMS (which we will add to redirectTo)
    ...
    const handleAuth = async (e) => {
        ...
        try {
            ...
            if (isLogin) {
                ...
                // 3. AUTO-CREATE INQUIRY (Trigger Card Creation on Dashboard)
                // We need a CID. If URL had one, use it. If not, generate distinct one.
                ...
                // Redirect back to chat with the established CID
                if (lawyerId) {
                ...
                // Construct Redirect URL with params to preserve context
                ...
                const { data, error: signUpError } = await supabase.auth.signUp({
                    email,
                    password,
                    // Redirect to this same page to handle confirmation flow if email is required
                    options: {
                ...
                // Handle Auto-Login vs Email Confirmation
                if (data.session) {
                    // Success (Auto-confirmed)
                    // If session exists, onAuthStateChange might trigger too? Be careful of double calls.
                    // But fetch is idempotent-ish.

                    // Let's rely on the manual push here for speed if session is immediate
                    const finalCid = cid || crypto.randomUUID();
                    const finalUser = data.user;

                    // CREATE INQUIRY IMMEDIATELY
                    if (lawyerId && finalUser) {
                        await fetch("/api/chat", {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                        ...
                        // If no lawyerId or user, just redirect to home or show success
                        router.push('/');
                    }
                } else {
                    // Confirmation email sent
                    setMessage("¡Cuenta creada! Revisa tu email para confirmar tu cuenta y acceder al chat.");
                    // DO NOT Redirect yet. Wait for them to click email link.
                    // The link will bring them back here (redirect logic needed in useEffect?)
                    setIsLogin(true);
                }
            }
